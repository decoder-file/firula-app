...
import { RadioGroup, Text, TextField, useTheme } from "@/design-system";
import type { UseCheckoutReturn } from "@/features/checkout/useCheckout";

/** Conteúdo puro (sem navegação) — embutido como card em `CheckoutScreen`. */
export function CustomFieldsStep({ checkout }: { checkout: UseCheckoutReturn }) {
  const { colors } = useTheme();
  const { relevantCustomFields, sharedCustomFieldValues, setSharedCustomFieldValues } = checkout;
